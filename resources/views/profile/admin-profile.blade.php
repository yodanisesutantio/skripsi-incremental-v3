        ...
        <div class="profile-header flex flex-col lg:flex-row gap-3 lg:gap-10 w-full items-center lg:justify-center">
            ...
            <div class="text-content text-center lg:text-left flex flex-col gap-1 lg:gap-2 w-full lg:w-[24rem] px-12 lg:px-0">
                @if (auth()->user()->age === NULL && auth()->user()->description === NULL)
                    <h1 class="font-encode tracking-tight font-semibold text-2xl lg:text-4xl">{{ auth()->user()->fullname }}</h1>
                ...
                @elseif (auth()->user()->description === NULL)
                    <h1 class="font-encode tracking-tight font-semibold text-2xl lg:text-4xl">{{ auth()->user()->fullname }}, {{ auth()->user()->age }}</h1>
                    <i class="font-league text-custom-grey/40 text-lg/tight lg:text-xl/tight">Belum ada deskripsi</i>
                @else
                    <h1 class="font-encode tracking-tight font-semibold text-2xl lg:text-4xl">{{ auth()->user()->fullname }}, {{ auth()->user()->age }}</h1>
                    <p class="font-league text-custom-grey text-lg/tight lg:text-xl/tight">{{ auth()->user()->description }}</p>
    ...
    <div id="delete-overlay" class="fixed hidden z-40 flex items-center justify-center top-0 left-0 w-full h-full bg-custom-dark/70">
        ...
        <div id="deleteConfirm" class="relative w-80 lg:w-[28rem] bottom-0 py-4 z-40 bg-custom-white rounded-xl">
            <div class="flex flex-row sticky px-5 bg-custom-white justify-between items-center pt-1 pb-4">
                <h2 class="font-league text-[27px]/none pt-1 lg:text-3xl font-semibold text-custom-dark ">Tutup Kursus?</h2>
            ...
            <div class="px-5 mt-2">
                <p class="font-league text-lg/snug lg:text-xl/tight text-custom-dark mb-1 lg:mb-12">Anda yakin ingin menutup dan menghapus lembaga kursus anda beserta semua data yang terkait didalamnya?</p>
            </div>
