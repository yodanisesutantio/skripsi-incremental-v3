...
@section('content')
    ...
    <div class="flex flex-col justify-center items-center bg-cover bg-center h-screen w-screen p-4" style="background-image: url('{{ asset('img/reset-password.webp') }}')">
        {{-- Glass Effect --}}
        <div class="flex flex-col p-6 lg:px-8 lg:py-6 w-full lg:w-[27rem] bg-center bg-custom-dark/40 border border-t-custom-white/25 border-b-custom-disabled-dark/20 border-r-custom-disabled-dark/20 border-l-custom-white/25 lg:gap-4 rounded-lg lg:rounded-2xl backdrop-blur-md">
            {{-- Form Header --}}
            <h1 class="text-3xl/tight lg:text-4xl text-center text-custom-white font-encode tracking-tight font-semibold">Atur Ulang Password</h1>

            {{-- Forms --}}
            <form action="{{ url('/reset-password/' . $user->username) }}" method="POST">
                @csrf
                <div class="flex flex-col mt-5 lg:mt-6 gap-5 lg:gap-7">
                    {{-- Input Password --}}
                    <div class="flex flex-col gap-1">
                        <label for="password" class="font-normal font-league text-lg text-custom-white">Masukkan Password Anda<span class="text-custom-destructive">*</span></label>
                    ...
                    {{-- Confirm Password --}}
                    <div class="flex flex-col gap-1">
                        <label for="password_confirmation" class="font-normal font-league text-lg text-custom-white">Ketik Ulang Password<span class="text-custom-destructive">*</span></label>
                        <div class="relative flex justify-end items-center">
                ...
                {{-- Submit Button --}}
                <div class="flex mt-7">
                    <button name="submit" type="submit" class="w-full py-3 rounded-lg lg:rounded-lg bg-custom-green hover:bg-custom-green-hover text-center lg:text-lg text-custom-white-hover font-semibold lg:order-2 duration-500">Simpan Password Baru</button>
                </div>
            </form>
