...
@section('content')
    ...
    {{-- When the payment haven't been uploaded, Show This --}}
    @if (!$enrollment->CoursePayment)
        <div class="mt-4 p-3 lg:p-5 bg-custom-destructive/15 w-full rounded-lg lg:rounded-xl">
            <h2 class="font-league font-normal text-lg/tight lg:text-xl/tight text-custom-destructive">Siswa belum mengunggah bukti pembayaran</h2>
        </div>
    {{-- When the payment is uploaded but haven't been verified, Show This --}}
    @elseif ($enrollment->CoursePayment && $enrollment->CoursePayment->paymentStatus === 0)
        <div class="mt-4 p-3 lg:p-5 bg-custom-destructive/15 w-full rounded-lg lg:rounded-xl">
            ...
            <div class="flex flex-col font-league text-custom-white gap-3 my-5 lg:my-8">
                <div class="grid grid-cols-2 gap-3">
                    ...
                    @if ($enrollment->schedule->isEmpty())
                        <a href="{{ url('/user-course/schedule/' . $enrollment->student_real_name . '/' . $enrollment['id'] . '/1') }}" class="col-span-2 w-full h-24 lg:h-32 bg-cover bg-center rounded-xl cursor-pointer" style="background-image: url('{{ asset('img/Course-Schedule-BG.webp') }}')">
                            {{-- Overlays --}}
                    ...
                    <div class="flex flex-col gap-3">
                        ...
                        {{-- Theory Guide Button --}}
                        <a href="{{ url('/user-course/theory/' . $enrollment['id'] . '/' . $currentMeetingNumber) }}" class="w-full h-32 lg:h-44 bg-cover bg-center rounded-xl cursor-pointer" style="background-image: url('{{ asset('img/Guide-BG.webp') }}')">
                            {{-- Overlays --}}
                            <div class="flex flex-col gap-0.5 justify-end p-2.5 bg-gradient-to-t from-custom-dark/80 from-15% to-custom-dark/30 to-70% w-full h-full rounded-xl lg:hover:bg-custom-dark/40 lg:hover:transition-colors lg:duration-500">
                                    ...
                                    <div class="flex flex-col gap-5 pt-2 lg:pt-1">
                                        @if ($i === 1)
                                            ...
                                            <div class="flex flex-row justify-between items-center">
                                                ...
                                                @if ($enrollment->schedule && !$enrollment->schedule->isEmpty())
                                                    {{-- Checked Checkbox --}}
    ...
    <div class="hidden flex-col lg:grid-cols-2 lg:items-center justify-center gap-6 fixed top-0 left-0 font-league w-full h-full bg-custom-dark/70 text-custom-white z-40 pt-12 lg:pt-0 px-6 lg:px-[4.25rem]" id="contact-other-party">
        ...
        <a href="{{ url('https://wa.me/' . $enrollment->instructor->phone_number) }}" target="_blank" id="contact-instructor" class="lg:w-full bg-cover bg-center rounded-xl lg:mt-10" style="background-image: url('{{ asset('storage/enrollment/profile_pictures/' . $enrollment->student_profile_picture) }}')">
        ...
        @endif
            ...
            {{-- Overlays --}}
            <div class="flex flex-col gap-2 w-full h-72 justify-center items-center rounded-xl px-6 pt-2 lg:pt-0 bg-custom-dark/75 lg:px-20 lg:hover:bg-custom-dark-low duration-300">
                <h2 class="font-semibold text-2xl/snug lg:text-4xl/snug">Hubungi Instruktur</h2>
                <p class="font-light text-base/tight lg:text-xl/tight text-center">Mulai percakapan dengan {{ $enrollment->instructor->fullname }}</p>
            </div>
        </a>
    </div>
    ...
    <script>
        ...
        $('#close-contact-other-party, #contact-admin, #contact-instructor').on('click', function() {
