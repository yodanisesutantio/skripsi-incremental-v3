...
@section('content')
    <div class="flex flex-col w-full h-full">{{-- Course Theory Content --}}
        ...
        <div class="hidden fixed bottom-0 flex-row flex-shrink-0 w-full px-3 pt-6 lg:pt-5 pb-4 items-center justify-between bg-custom-white select-none z-30" id="nav-wrapper">
            {{-- Done Quiz Button --}}
            <form action="{{ url('/user-course/quiz/' . $enrollment['id'] . '/' . $meeting_number) }}" id="done-quiz" class="hidden" method="POST">
                @csrf
        ...
        const swiper = new Swiper('.swiper', {
            ...
            on: {
                slideChange: function () {
                    ...
                    if (this.isEnd) {
                        $('#done-quiz').removeClass('hidden'); // Unhide the submit button
                    } else {
                        $('#done-quiz').addClass('hidden'); // Hide the submit button if not on the last slide
                    }
                }
            }
        });
